...
únicos decidíveis, porque o degrau alfabético deles só pode vir do preset. As duas famílias são
listas separadas desde a ADR 0030: `rounded-xs` continua real enquanto `p-xs` morreu, então o banimento
de espaço carrega a régua de tamanho inteira — `2xs … 3xl`, toda grafia `sp-`, e as extrapolações —
e é isso que impede o rename de voltar uma classe por vez. Os utilitários de
