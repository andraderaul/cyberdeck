...
byte** — including `index.css` and `tailwind.config.js`. This is the trigger ADR 0011 named. ADR 0011
stays as the historical record of why we waited; this ADR records that the condition fired and scopes
...
generic browser-plumbing (operational-error mechanism, canvas Recording). It is deliberately *not* a
domain core — each app's pipeline (ASCII conversion, glitch Effects) stays in the app.

Only the modules with two real callers and an empty diff cross the seam. Three extractions:
...
- **Extract three packages up front (`tokens` / `ui` / `hooks`).**
  - *Rejected because:* no app exercises the seams between them yet; premature per-category versioning
    and build config for a hypothetical split.
- **Pre-build the package to `dist` with its own bundler.**
  - *Rejected because:* it forces build ordering, watch mode, and artifact caching — the tooling
    weight ADR 0011 declined. Ship-source produces an identical app bundle.
- **Extract the whole webcam and output modules too.**
  - *Rejected because:* it drags app vocabulary through one interface, the exact "force different
    tools through one abstraction" failure ADR 0011 warns of.

## Consequences

**Positive:**
- Each app's `index.css` and `tailwind.config.js` shrink to an import + a preset spread + the package
  `content` glob.
- Future reviews scope against "empty diff + two callers," not against "any duplication."

**Negative:**
- That `content` glob is a standing requirement for any app on the deck. It is the kit's one
  non-obvious interface constraint, and it lives in the package README and above.
...
- A real npm workspace at `packages/deck-kit`, `"private": true`, consumed **as source**: its
  ...
  build-ordering — this keeps the "light tooling, no Nx/Turborepo" stance of ADR 0011. The trade: the
  package is not consumable outside this monorepo, which is fine — nothing else consumes it.
- A single package, not three (`tokens` / `ui` / `hooks`). Today both apps consume every category
  together, so splitting now would create three hypothetical seams for a variation nobody exercises.
  If a third program wants one category without the others, that seam becomes real and we split then.
- Subpath exports by category — `@cyberdeck/deck-kit/{ui,hooks,utils,errors,recording}`, plus
...
- The package owns the visual language. `index.css` and `tailwind.config.js` have an empty diff: they
  *are* the deck's shared language (CONTEXT-MAP). The kit exports `tokens.css` (the CSS custom
...
- **`use-webcam-state`** — the shared body (~40 lines of `getUserMedia` + track lifecycle) is wrapped
  in an interface saturated with domain vocabulary: `mode` is the *argument the caller passes*
  (`switchMode('live' | 'webcam')`), and `Effect` is a reserved domain word in GLITCH (a pure
  `PixelBuffer` transform), so a neutral core could name its actions neither app's way. Extracting
  only the imperative controller would split `startWebcam`'s getUserMedia → dispatch → callback story
  across a package seam — a *locality loss* for a thin DRY win. The mirror / no-mirror divergence is
  load-bearing (ASCII mirrors the selfie view; GLITCH cannot, because its canvas *is* the output).
  Left copied — this is ADR 0011 being right.
- **`output.ts`** (beyond `mimeToExtension`) — `outputFilename` and `OutputKind` are almost entirely
  ...
  without dragging vocabulary across the seam. `planPngExport` / `MAX_EXPORT_DIM` are ASCII-only (one
  caller). Left in each app.
- **App-specific `ui/` primitives** — `badge`, `modal`, `tooltip`, `header-button`, `error-text` have
