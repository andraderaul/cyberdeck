...
(`◈ ◉ ◎ ● ○ ⬆ ↑ ✕ × ⚿ ▸ ⇄ ⏺ ⏹ ⚠`, never emoji), the 44x44 targets, and the rule that elevation is
1px borders and a lighter fill, never a shadow.
...
| **`EXPORT_TERMINAL` overlay** | **ADR 0013:** overlays own their background, and the backdrop here is the user's own artwork. Export is already a Strip tab (**ADR 0020**). |
| **Bottom nav with icon-only controls** | Duplicates the Strip's tabs, and icon-only controls over the canvas collide with the `ICON_GLYPH_SIZE` rule: chrome over the canvas stays at its drawn size and buys its 44px as an overlay precisely so the picture is not charged for its own chrome (**ADR 0013**). |
| **Preset badges over the artwork** | **ADR 0013** again. |
...
| **The mock's toast for a refused Chain import** | The app's existing refusal path is *more* careful than the mock's, not less — `chain-codec.ts` compares `CHAIN_FILE_VERSION` by exact equality and reads only `type` and `params`. The one place the mock is behind the app. |
