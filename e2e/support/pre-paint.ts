...
// blocking script in each themed workspace's `index.html`, where it stamps `data-theme` before React
...
//
// Four workspaces carry that copy, and the assertion is the same shape in all four, so it lives here
// rather than four times over. The kit's roster guard already reads the four scripts as *text* and
// pins the roster, the key and the attribute they name (`theme/roster.test.ts`); what only a browser
// can add is the half below — that the stamp really lands, and lands before anything paints.
...
/**
 * That `data-theme` is on the root element before the first paint, in two halves — each alone passes
 * over a build the other catches.
 */
export async function expectStampedBeforeFirstPaint(
  page: Page,
...
/**
 * That a Theme picked in the header is still the Theme after a reload — in the attribute, in the
 * trigger's accessible name and in `localStorage`, which is the only one of the three the pre-paint
 * script actually reads.
 */
export async function expectPickedThemeSurvivesReload(page: Page, theme = 'chiba'): Promise<void> {
  await page.goto('/')
