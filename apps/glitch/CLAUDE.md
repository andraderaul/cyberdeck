...
the animated Seed (#311) and the per-Link bypass (#371). All eight Effects are live — Source Image
*or* Live Source → the Chain → PNG Export / Capture / Copy / Recording — the pure-core /
imperative-shell seam is established, and the render is deterministic in Chain + Seed, which is
what lets a Live Source animate by advancing the Seed alone. The front door is the curated Presets
plus Randomize; behind the EDIT tab the Chain is fully editable — reorder, add, remove, duplicate,
bypass, the same Effect more than once. A Chain built by hand exports as JSON and comes back
(**Chain JSON**, `CONTEXT.md`), which is the only way structural variety reaches the app from
