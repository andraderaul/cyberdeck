...
- 5df832a: GOLEM//Console installs, and it runs with the network off. A web app manifest makes the machine
  installable under its own mark, and a service worker precaches the whole shell on the first visit, so
  the second one opens with no network and you can write assembly, assemble it, and drive a run from
  the Console exactly as before. The program you were last editing is still in the editor: your Source
  has always lived in this browser and neither installing nor an outage touches it. Installed, it opens
  standalone on the same near-black the page paints.

  A new version never takes over a session in progress — which matters more here than anywhere else on
  the deck, because a running Machine, its Terminal and its Registers exist nowhere but the tab. A new
  build installs quietly behind the one you are using and waits: the run carries on, the Clock keeps
  its rate, and nothing changes until you open the program again or take the offer that appears under
  the header.

### Patch Changes

- bf2b5e9: `share` compresses a Source into the URL fragment, so a link is this program's artifact — and until
  now that link previewed as nothing. GOLEM//Console gets its own favicon (the Console prompt, which
  is the whole control grammar), the icon sizes a browser tab, an iOS home screen and an install
  prompt each ask for, a description written from the reader's side, and an Open Graph / Twitter card
  showing a session beside the live register readout and the Terminal. `theme-color` matches the Theme
  the pre-paint script paints, so the browser chrome no longer flashes a different colour.
...
- 9e97a4c: `clear` wipes the Console log, and `ctrl+l` / `cmd+k` do it without leaving the prompt — the two
...
- f103199: GOLEM//Console's Theme control stops cycling and becomes a popover (ADR 0024): the header trigger
...
- abed3c7: GOLEM//Console gains the deck's Theme control, in the header (ADR 0024). It is the tracer for
...
- f1fb002: Move the Console's control surface behind one pure reducer. The 18-arm command dispatch, its
...
- e62d902: Spell an instruction's field layout and its opcode→mnemonic mapping once. The machine's `execute`
...
- b999b8a: Concentrate every cache classification behind one memory-access surface. `fetch`, `load`/`store`
...
- 3436624: GOLEM//Console v3 — the cache, a classifier-lens over execution. On by default, it classifies every memory access as a Hit or a Miss bit-for-bit against the unit-3 oracle (Harvard I/D caches, 8 Lines × 2 Sets, 4-word blocks, LRU-by-AGE) without ever changing a value or costing a cycle. The CACHE panel spotlights the Line each Step touches — its two Sets, HIT/MISS coloured, an eviction happening on screen — with the other Lines as a warming strip; the `cache` command toggles the lens; the Console narrates the hit/miss boletim at the end of a run; and `load memory_access` plays the signature scene. The last Poxim unit.
