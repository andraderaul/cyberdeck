...
// breakpoint and the same `hidden sm:block` the subtitle beside it already uses. That pair of
// classes lives in the kit with the control now (#400) rather than here, because the mute is one
// component serving four headers — but this is the header it was measured in, so the table stays.
//
...
//
// **#400 re-measured the same three widths after the control crossed the seam and lost this file's
// display face for `HeaderButton`'s own: 372 at all three, unchanged.** Below `sm` the mute is
// glyph-only and lands on `HeaderButton`'s `min-w-[44px]` floor, so the face it wears cannot move
// the row — and that floor is also why the target stays real here rather than needing
// `touch-target.ts`. The other three headers were measured in the same run and none of them
// overflows at any of the three widths: the mute costs each of them exactly its 44px in a row that
// had the room. This is the tight one on the deck.
