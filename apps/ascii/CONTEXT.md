...
**Color Mode**:
O esquema de colorização aplicado ao canvas. Paletas temáticas (`matrix`, `bw`, `retro`, `sepia`, `neon`) pintam todos os caracteres com uma cor fixa; as duplas (`synthwave`, `matrix-dual`, `acid`, `infrared`) escolhem entre duas conforme a luminosidade da célula. O modo `original` usa o RGB de cada pixel da imagem original, e o `adaptive` tira a paleta da própria **Source Image** (ver abaixo).
...
**Adaptive** (Color Mode):
O único **Color Mode** que não traz cor nenhuma consigo: a paleta é *quantizada* da própria entrada, e a arte volta pintada nas cores que ela já tinha. O cubo de cor é cortado numa grade fixa de 4×4×4 bins, cada **AsciiCell** que pinta cai num bin, e cada bin ocupado vira uma cor — a *média* do que caiu nele, e não o canto do bin, então as cores existem mesmo na imagem. Uma célula recebe a cor do bin em que ela *está*, nunca a entrada de que ela está mais perto: essa distinção é o desenho inteiro, e o preço dela é que a paleta não tem teto de seis, tem o teto da grade — no máximo 64 cores, na prática algumas dezenas. É uma posterização, não uma redução a um punhado, e é isso que uma partição que não se mexe custa.
Células em branco não contam: elas não pintam nada, e é justamente isso que faz a grade inteira e a grade recortada à fit region (ADR 0010) derivarem *a mesma* paleta, porque as bandas de letterbox são células vazias — sem isso o preview e os Exports de texto seriam duas quantizações de duas imagens diferentes. Como a paleta sai da grade amostrada, e não da **Source Image** em pixels, mexer na **Resolution** reamostra a grade e re-deriva a paleta: as médias andam um pouco, o que é contínuo e discreto, não um salto.
Recalculado **a cada frame**, inclusive no **Live Source**, e a razão é a assimetria entre os dois modos de errar. Segurar a paleta por Source não sobrevive: um Live Source não tem "Source" para segurar paleta nenhuma — a cena *é* a entrada, e os primeiros frames de uma webcam são os que ela gasta esquentando e fechando o balanço de branco, de modo que a sessão inteira ficaria pintada nas cores de um frame escuro e errado. Já o tremor entre frames é o preço de uma partição *dependente dos dados*, e esta não é uma: as bordas dos bins são constantes, decididas antes de qualquer imagem. Entre dois frames uma célula só muda de cor de duas maneiras, e as duas são limitadas — ou ela cruza um limiar fixo de canal, e aí anda sozinha para o bin vizinho, ou a média do próprio bin dela se desloca, o que é uma média sobre a população daquele bin e não alcança bin nenhum além dele. Não existe passo em que a mudança de uma célula repinte uma célula de outro bin.
A forma recusada está registrada porque ela *parece* a mesma ideia: guardar os seis bins mais populosos e pintar cada célula com o mais próximo dos seis é um Voronoi sobre seis médias dependentes dos dados, ou seja, a partição volta a se mexer com a imagem. Mudar uma única célula de mão trocava o corte e repintava boa parte da grade, inclusive bins que aquela célula nunca tocou. A medição mora no JSDoc de `quantizePalette` (`palette.ts`), e `palette.test.ts` guarda a reprodução como regressão — aqui fica só a regra: a cor de uma célula sai do bin em que ela *está*.
_Avoid_: auto color, paleta automática, k-means, median cut, nearest-of-N (são os mecanismos recusados, não o termo), Theme
...
- **Color Mode × Theme** colidem em português e em inglês — os dois são "o esquema de cores". São coisas distintas e este é o único programa onde os dois controles ficam à vista: **Color Mode pinta a arte do usuário; Theme pinta a casca do deck** (ADR 0024, que reusa a fronteira da ADR 0013). Resolvido também no nome: nenhum dos sete Themes se chama `matrix` ou `neon` justamente porque esses dois já são Color Modes daqui. O `adaptive` é a forma mais forte que essa fronteira podia tomar: toda cor que ele produz saiu da **Source Image**, então não há constante nenhuma no caminho para alguém trocar por um token de Theme.
- **`adaptive` (Color Mode) × "adaptive density" (ADR 0020)** colidem, e colidem dentro do mesmo arquivo: `settings-editor.tsx` fala das duas coisas. São eixos diferentes — o Color Mode adapta a *cor* à **Source Image**; a adaptive density adapta o *layout* do Control Strip à largura da tela, e não tem nada a ver com a arte. Não resolvido no nome, ao contrário do que a ADR 0024 fez com os Themes: `adaptive` é como a issue e a UI já chamam o modo, e renomear um controle visível para desfazer uma colisão com um termo de layout sairia mais caro do que declará-la. Ao escrever, qualifique: "o Color Mode `adaptive`" e "a densidade adaptativa do Strip", nunca "adaptive" sozinho.
