...
This app lives in the [CYBERDECK](../../README.md) monorepo. Install once from the repo root — npm workspaces links every app.

```bash
# from the repo root: install dependencies for the whole deck
npm install

# start this app's development server
npm run dev --workspace @cyberdeck/ascii
```
...
From this directory (`apps/ascii`):
...
Linting and formatting are repo-wide — run `npm run check` from the root.
...
2. Make your changes — run `npm run test:run` and `npm run check` before pushing
