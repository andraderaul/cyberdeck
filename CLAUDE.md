...
**Name a scale step the preset defines.** The same silent failure, one layer over: `gap-2xs` is a
key, `gap-3xs` is not, and Tailwind answers an undefined step by generating no class at all — no
error from Tailwind, tsc or Biome. The scale runs `2xs · xs · sm · md · lg · xl · 2xl · 3xl` (plus
`sp-*` for section macro spacing) and does **not** extrapolate: there is no `3xs` and no `4xl`. The
kit's scale guard fails the build the same way the hue guard does, with the class, the file and the
line. Note `--gap-xs` is 4px and `--gap-2xs` is 6px — `xs` is the *tighter* of the two, against the
pattern the rest of the scale follows.
